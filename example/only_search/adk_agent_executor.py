        ...
        async for event in self._run_agent(session_id, new_message):
            ...
            if agent_author in self.show_agent:
                ...
                if event.content and event.content.parts:
                    ...
                    last_validation = final_session.state.get("last_validation", {})
                    # 最后一个agent的输出了，输出成status
                    await task_updater.update_status(
                        TaskState.working,
                        message=task_updater.new_agent_message(
                            convert_genai_parts_to_a2a(event.content.parts), metadata={"author": agent_author, "show": True, "references": references, "last_validation": last_validation}
                        ),
                    )
